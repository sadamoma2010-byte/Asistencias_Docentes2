...
def calcular_diferencia_minutos(hora_real, hora_esperada):
    ...
    """
    return hora_a_minutos(hora_real) - hora_a_minutos(hora_esperada)
...
def evaluar_puntualidad(hora_real, hora_esperada, tolerancia):
    ...
    """
    diferencia = calcular_diferencia_minutos(hora_real, hora_esperada)
    ...
    try:
        ...
        cursor.execute(
            ...
            SELECT
                ...
                u.bloqueado_hasta, u.debe_cambiar_contrasena, r.nombre AS rol_nombre
            FROM usuarios u
        ...
        if usuario["bloqueado_hasta"]:
            ...
            if usuario["bloqueado_hasta"] > ahora:
                return jsonify({"status": "error", "message": "La cuenta está bloqueada temporalmente."}), 403
...
def registro():
    ...
    conn = get_db_connection()
    if not conn:
        return jsonify({"status": "error", "message": "No fue posible conectar con PostgreSQL."}), 500

    cursor = None
    try:
        cursor = conn.cursor(cursor_factory=RealDictCursor)
        cursor.execute(
            "SELECT id FROM usuarios WHERE correo = %s OR documento = %s LIMIT 1",
            (correo, documento)
        )
        if cursor.fetchone():
        ...
        rol = cursor.fetchone()
        if not rol:
            return jsonify({"status": "error", "message": "No existe el rol DOCENTE."}), 500
...
# ============================================================
# API - MARCAR ASISTENCIA (CORREGIDA Y COMPLETA)
# ============================================================
...
def marcar_asistencia():
    ...
    conn = get_db_connection()
    if not conn:
        return jsonify({"status": "error", "message": "Error conectando con PostgreSQL."}), 500
    ...
    try:
        ...
        return jsonify({"status": "error", "message": "Ocurrió un error al registrar la asistencia."}), 500
    finally:
        if cursor:
            cursor.close()
        conn.close()
...
# ============================================================
# REGISTRO DE BLUEPRINTS Y EJECUCIÓN
# ============================================================

# Importar y registrar el módulo de permisos
from permisos import permisos_bp
